...
def eval(checkpoint_path, dataroot):
    ...
    for index, batch in enumerate(tqdm(valloader)):
        ...
        with torch.no_grad():
            ...
            with open("answer_chatgpt_gt_shubham.json", "w") as f:
                json.dump(objects_chatgpt_json, f, indent=4)
            for matched_img_ind, matched_img in enumerate(matched_imgs):
                np.save(os.path.join(f"imgs/qualitative_shubham/{matched_img_ind + 1}_matched_img.npy"), matched_img)
                Image.fromarray(matched_img).save(os.path.join(f"imgs/qualitative_shubham/{matched_img_ind + 1}_matched_img.png"))
