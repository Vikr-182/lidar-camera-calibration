...
class FuturePredictionDataset(torch.utils.data.Dataset):
    ...
    def get_input_data(self, rec):
        ...
        for cam in cameras:
            ...
            im = Image.open(osp.join(self.nusc.dataroot, camm['filename']))
            ...
            xs = [60, 61, 61, 61, 61, 62, 62, 62, 62, 62, 62, 63, 63, 63, 63, 63, 63,
                ...
                67, 67, 67, 67, 67, 67, 67, 68, 68, 68, 68, 68, 68, 68, 68, 68, 68,
                69, 69, 69, 69, 69, 69, 69, 69, 69, 69, 70, 70, 70, 70, 70, 70, 70,
                70, 70, 70, 70, 71, 71, 71, 71, 71, 71, 71, 71, 71, 71, 72, 72, 72,
                72, 72, 72, 72, 72, 72, 72, 73, 73, 73, 73, 73, 73, 73, 73, 73, 73,
                73, 74, 74, 74, 74, 74, 74, 74, 74, 74, 74, 75, 75, 75, 75, 75, 75,
                75, 75, 76, 76, 76, 76, 76, 76, 76, 77, 77, 77, 77, 77, 78, 78, 78,
                79]
            
            ys = [71, 70, 71, 72, 73, 69, 70, 71, 72, 73, 74, 69, 70, 71, 72, 73, 74,
                75, 68, 69, 70, 71, 72, 73, 74, 75, 76, 77, 69, 70, 71, 72, 73, 74,
                75, 76, 77, 78, 70, 71, 72, 73, 74, 75, 76, 77, 78, 79, 72, 73, 74,
                75, 76, 77, 78, 79, 80, 81, 73, 74, 75, 76, 77, 78, 79, 80, 81, 82,
                74, 75, 76, 77, 78, 79, 80, 81, 82, 83, 75, 76, 77, 78, 79, 80, 81,
                82, 83, 84, 85, 77, 78, 79, 80, 81, 82, 83, 84, 85, 86, 78, 79, 80,
                81, 82, 83, 84, 85, 86, 87, 79, 80, 81, 82, 83, 84, 85, 86, 87, 88,
                89, 81, 82, 83, 84, 85, 86, 87, 88, 89, 90, 82, 83, 84, 85, 86, 87,
                88, 89, 83, 84, 85, 86, 87, 88, 89, 84, 85, 86, 87, 88, 86, 87, 88,
                87]
            ...
            if cam == "CAM_BACK":
                for ppc in ppcs:
                    ...
                    points = view_points(ppc.points[:3, :], np.array(cs_record['camera_intrinsic']), normalize=True)
        ...
        images, intrinsics, extrinsics, unnormalized_images, rotations, translations, camera_intrinsics, point_clouds = (torch.cat(images, dim=1),
        ...
        return images, intrinsics, extrinsics, depths, unnormalized_images, rotations, translations, camera_intrinsics, point_clouds
    ...
    def __getitem__(self, index):
        ...
        keys = ['image', 'intrinsics', 'extrinsics', 'depths',
                ...
                'rotations', 'translations', 'camera_intrinsics', 'point_clouds'
                ]
        for key in keys:
            data[key] = []
        ...
        for i, index_t in enumerate(self.indices[index]):
            ...
            if i < self.receptive_field:
                images, intrinsics, extrinsics, depths, unnormalized_images, rotations, translations, camera_intrinsics, point_clouds = self.get_input_data(rec)
            ...
            if i == self.cfg.TIME_RECEPTIVE_FIELD-1:
                ...
                if self.cfg.DENOISE:
                    gt_trajectory_prev = denoise(gt_trajectory)
                else:
                    gt_trajectory_prev = gt_trajectory
                data['gt_trajectory_prev'] = torch.from_numpy(gt_trajectory_prev).float()
                # traj=torch.tensor(gt_trajectory_prev)[:, :2].reshape(1,9,2)
                gt_trajectory = gt_trajectory[2:]
                # denoised = torch.tensor(gt_trajectory).float()
                # gt_traj = torch.zeros_like(torch.from_numpy(gt_trajectory).float()).float()
                # gt_traj[:, :2] = denoised
                if self.cfg.DENOISE:
                    data['gt_trajectory'] = gt_traj
                    plt.scatter(gt_traj[:, 0], gt_traj[:, 1])
                    plt.savefig("output_vis/debug_dataloder.png");plt.clf()
                else:
                    data['gt_trajectory'] = gt_trajectory
                data['command'] = command
                trajs = self.get_trajectory_sampling(rec)
                data['sample_trajectory'] = torch.from_numpy(trajs).float()

            if self.cfg.PLANNING.DENSE.ENABLED and i == self.cfg.TIME_RECEPTIVE_FIELD-1:
                """
                    ITERATE OVER CENTERLINES
                """
                final_cen = torch.zeros((1, 10, 2000, 2))
                centerlines = self.generate_centerlines(rec, map_name)
                centerlines = torch.tensor(centerlines)
                num_centerlines = centerlines.shape[0]
                if num_centerlines>10:
                    centerlines = centerlines[:10,:,:]
                centerlines = centerlines.unsqueeze(0)
                if num_centerlines == 0:
                    # extend gt on both sides to create centerline.
                    cx = extend_both_sides(gt_trajectory_prev, resolution=0.1, extension=30)
                    cx = interp_arc_np(2000, cx) # (2000, 2)
                    cx = torch.tensor(cx)
                    centerlines = torch.zeros((1, 2000, 2))
                    centerlines[0] = cx
                    num_centerlines = 1
                final_cen[:,:num_centerlines,:,:] += centerlines[:, :2000]

                data['centerlines'] = final_cen
                data['num_centerlines'] = num_centerlines
