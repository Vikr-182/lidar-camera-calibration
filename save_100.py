...
def eval(checkpoint_path, dataroot):
    ...
    for index, batch in enumerate(tqdm(valloader)):
        ...
        if index%20==0:
            ...
            print(index)
            os.mkdir(f'data_21_aug/{index}')

            for idx in range(6):
                img = cv2.cvtColor(batch['unnormalized_images'][0,2,idx].numpy(), cv2.COLOR_RGB2BGR)
                cv2.imwrite(f'data_21_aug/{index}/{idx}.png', img)

            with open(f'data_21_aug/{index}/point_clouds.pkl', 'wb') as f: 
                pickle.dump(batch['point_clouds'], f)
            
            with open(f'data_21_aug/{index}/point_clouds_labels.pkl', 'wb') as f: 
                pickle.dump(batch['point_clouds_labels'], f)
            
            with open(f'data_21_aug/{index}/point_clouds_panoptic_labels.pkl', 'wb') as f: 
                pickle.dump(batch['point_clouds_panoptic_labels'], f)

            with open(f'data_21_aug/{index}/panoptic_mappings_list.pkl', 'wb') as f: 
                pickle.dump(batch['panoptic_mappings_list'], f)

            cv2.imwrite(f'data_21_aug/{index}/bev.png', 255*batch['segmentation'][0,2,0].numpy())
