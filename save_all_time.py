...
def eval(checkpoint_path, dataroot):
    ...
    for index, batch in enumerate(tqdm(valloader)):
        ...
        # 6 CAM IMAGES             -  unnormalized_images           -  done
        # LIDAR                    -  point_clouds                  -  done
        # LIDAR LABEL              -  point_clouds_labels           -  done
        # LIDAR PANOPTIC LABEL     -  point_clouds_panoptic_labels  -  done
        # LIDAR PANOPTIC MAPPINGS  -  panoptic_mappings_list        -  done
        # BEV                      -  segmentation                  -  done
        ...
        for idx in range(6):
            img = cv2.cvtColor(batch['unnormalized_images'][0,2,idx].numpy(), cv2.COLOR_RGB2BGR)
            cv2.imwrite(os.path.join(save_path, str(cur_scene_token[0]) + "_" + "{0:0=6d}".format(index), '{cam_names[idx]}.png'), img)

        with open(os.path.join(save_path, str(cur_scene_token[0]) + "_" + "{0:0=6d}".format(index), 'point_clouds.pkl'), 'wb') as f: 
            pickle.dump(batch['point_clouds'], f)
        
        with open(os.path.join(save_path, str(cur_scene_token[0]) + "_" + "{0:0=6d}".format(index), 'point_clouds_labels.pkl'), 'wb') as f: 
            pickle.dump(batch['point_clouds_labels'], f)
        
        with open(os.path.join(save_path, str(cur_scene_token[0]) + "_" + "{0:0=6d}".format(index), 'point_clouds_panoptic_labels.pkl'), 'wb') as f: 
            pickle.dump(batch['point_clouds_panoptic_labels'], f)

        with open(os.path.join(save_path, str(cur_scene_token[0]) + "_" + "{0:0=6d}".format(index), 'panoptic_mappings_list.pkl'), 'wb') as f: 
            pickle.dump(batch['panoptic_mappings_list'], f)

        cv2.imwrite(os.path.join(save_path, str(cur_scene_token[0]) + "_" + "{0:0=6d}".format(index), 'gt_bev.png'), 255*batch['segmentation'][0,2,0].numpy())
