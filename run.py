...
def eval(checkpoint_path, dataroot):
    ...
    for index, batch in enumerate(tqdm(valloader)):
        ...
        np.save("grad_data/arr.npy", arr)
        np.save("grad_data/unn.npy", batch["unnormalized_images"].squeeze())
        np.save("grad_data/rotations.npy", batch["rotations"].squeeze())
        np.save("grad_data/translations.npy", batch["translations"].squeeze())
        np.save("grad_data/camera_intrinsics.npy", batch["camera_intrinsics"].squeeze())
        np.save("grad_data/point_clouds.npy", batch["point_clouds"][-1].squeeze())
