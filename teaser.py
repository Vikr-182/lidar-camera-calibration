...
def eval(checkpoint_path, dataroot):
    ...
    valdata.indices = valdata.indices[33:]
    ...
    for index, batch in enumerate(tqdm(valloader)):
        ...
        with torch.no_grad():
            ...
            arr = np.ones((200, 200, 3))
            whe = np.where(batch['hdmap'].squeeze()[2,1] > 0)
            arr[whe[0], whe[1]] = np.array([204,153,255])/256
            whe = np.where(batch['segmentation'].squeeze()[2] > 0)
            # whe = np.where(pred > 0)
            arr[whe[0], whe[1]] = np.array([0,0,255])/256
            barr = np.copy(arr)
            varr = np.copy(arr)

            arr = np.ones((200, 200, 3))
            whe = np.where(batch['hdmap'].squeeze()[2,1] > 0)
            arr[whe[0], whe[1]] = np.array([0,0,0])/256
            whe = np.where(batch['hdmap'].squeeze()[2,0] > 0)
            arr[whe[0], whe[1]] = np.array([84,68,70])/256
            whe = np.where(batch['segmentation'].squeeze()[2] > 0)
            # whe = np.where(pred > 0)
            arr[whe[0], whe[1]] = np.array([0,0,255])/256
            barr = np.copy(arr)
            ...
            plt.imshow(varr[50:150, 50:150])
            plt.axis("off")
            plt.grid(False)
            plt.savefig("imgs/qualitative_anushka/sample_bev.png", bbox_inches="tight", dpi=200)
            ...
            for j in range(6):
                img = batch['unnormalized_images'][0,2,j].numpy()
                
                user_message = "Describe this image."
                conv = reset_conv()
                answer = llava_inference(model_llava, image_processor, tokenizer, conv, user_message, img, device);
                imc[cam_keys[j]] = {}
            ...
            for idx in tqdm(range(1, labels)):
                ...
                try:
                    ...
                    print("no lidar point close ", idx)
                    continue
                ...
                # conv = reset_conv()
                # llm_message_llava = llava_inference(model_llava, image_processor, tokenizer, conv, user_message, img_cropped, device);
                # obj['llm_message_llava_rear'] = llm_message_llava

                # user_message = "Is there any text written on the object? Please look carefully, and describe it."
                # conv = reset_conv()
                # llm_message_llava = llava_inference(model_llava, image_processor, tokenizer, conv, user_message, img_cropped, device);
                # obj['llm_message_llava_text'] = llm_message_llava

                obj["bg_description"] = imc[matched_cam]["description"]
                # obj["llava_bg_unusual"] = imc[matched_cam]["unusual"]
                obj["weather"] = imc[matched_cam]["weather"]

                objects_json.append(obj)
                # objects_chatgpt_json.append(obj2)

            with open("imgs/qualitative_anushka/answer_gt.json", "w") as f:
            ...
            for cimg_ind, cimg in enumerate(cam_imgs): np.save(os.path.join(f"imgs/qualitative_anushka/{cimg_ind + 1}_cimg.npy"), cam_img); Image.fromarray(cimg).save(os.path.join(f"imgs/qualitative_anushka/{cimg_ind + 1}_cimg.png"))
            plt.figure(figsize=(10, 10))
            plt.imshow(varr)
            plt.axis("off")
            plt.grid(False)
