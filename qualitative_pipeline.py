...
def eval(checkpoint_path, dataroot='/raid/t1/scratch/vikrant.dewangan/datas/'):
    ...
    valdata.indices = valdata.indices[15899:]
    ...
    for index, batch in enumerate(tqdm(valloader)):
        ...
        try:
            os.mkdir(f'vid_s6/{15899+index}/')
        except:
            pass

        cv2.imwrite(f'vid_s6/{15899+index}/cam_0.png', batch['unnormalized_images'][0,2,0].numpy()[:,:,::-1])
        cv2.imwrite(f'vid_s6/{15899+index}/cam_1.png', batch['unnormalized_images'][0,2,1].numpy()[:,:,::-1])
        cv2.imwrite(f'vid_s6/{15899+index}/cam_2.png', batch['unnormalized_images'][0,2,2].numpy()[:,:,::-1])
        cv2.imwrite(f'vid_s6/{15899+index}/cam_3.png', batch['unnormalized_images'][0,2,3].numpy()[:,:,::-1])
        cv2.imwrite(f'vid_s6/{15899+index}/cam_4.png', batch['unnormalized_images'][0,2,4].numpy()[:,:,::-1])
        cv2.imwrite(f'vid_s6/{15899+index}/cam_5.png', batch['unnormalized_images'][0,2,5].numpy()[:,:,::-1])

        np.save(f'vid_s6/{15899+index}/segmentation.npy', batch['segmentation'].numpy())
        np.save(f'vid_s6/{15899+index}/hdmap.npy', batch['hdmap'].numpy())
