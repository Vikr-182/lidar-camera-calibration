...
from src.models.stp3 import STP3
from src.losses import SpatialRegressionLoss, SegmentationLoss, HDmapLoss, DepthLoss
from src.metrics import IntersectionOverUnion, PanopticMetric, PlanningMetric
from src.utils.geometry import cumulative_warp_features_reverse, cumulative_warp_features
...
class TrainingModule(pl.LightningModule):
    ...
    def shared_step(self, batch, is_train):
        ...
        if is_train:
            ...
            if self.cfg.PLANNING.ENABLED:
                ...
                pl_loss, final_traj, CS,_, gt_cost_fc, gt_cost_fo = self.model.planning(
        ...
        # Metrics
            ...
            if self.cfg.PLANNING.ENABLED:
                ...
                _, final_traj, CS,_, gt_cost_fc, gt_cost_fo = self.model.planning(
                    cam_front=output['cam_front'].detach(),
                    trajs=trajs[:, :, 1:],
                    gt_trajs=labels['gt_trajectory'][:, 1:],
    ...
    def prepare_future_labels(self, batch):
        """
        (2 past, 1 present, 4 future)
        segmentation_labels: B, 7, 1, 200, 200 -> ['vehicle'] for the 7 times frames.
        hdmap_labels: B, 7, 2, 200, 200 -> ['lane divider', 'drivable_area'] for the 7 time frames.
        future_egomotion: B, 7, 6 -> denotes the transformation matrix for the 7 time frames 
        gt_trajectory: B, 5, 3 -> denotes the GT for current and 4 frames into the future
        """
        ...
        """
        only present frame's hdmap 
        labels['hdmap']: B, 2, 200, 200. long-> converts to [0, 1] values from [0.0, 1.0]. contiguous -> just ensures in same memory.
        """
        ...
        # Warp past labels to present's reference frame
        """
        ...
        segmentation_labels_past: B, 2, 1, 200, 200
        """
        segmentation_labels_past = cumulative_warp_features(
            x=segmentation_labels[:, :self.model.receptive_field].float(),
            flow=future_egomotion[:, :self.model.receptive_field],
            mode='nearest', spatial_extent=self.spatial_extent,
        ).long().contiguous()[:, :-1]
        
        # Warp future labels to present's reference frame
        """
        x[:, 0] remains unchanged
        x[:, 1] is warped using flow[:, 0].inverse()
        x[:, 2] is warped using flow[:, 0].inverse() @ flow[:, 1].inverse()        
        segmentation_labels: B, 5, 1, 200, 200
        """        
        segmentation_labels = cumulative_warp_features_reverse(
            x=segmentation_labels[:, (self.model.receptive_field - 1):].float(),
            flow=future_egomotion[:, (self.model.receptive_field - 1):],
            mode='nearest', spatial_extent=self.spatial_extent,
        ).long().contiguous()
        ...
        if self.cfg.INSTANCE_SEG.ENABLED:
            """
            gt_instance: B, 7, 200, 200
            instance_center_labels: B, 7, 1, 200, 200
            instance_offset_labels: B, 7, 2, 200, 200
            gt_instance_past: B, 2, 200, 200 -> past warped
            gt_instance: B, 5, 200, 200 -> future warped
            """
...
if __name__ == "__main__":
    ...
    for _, data in enumerate(trainloader):
        # model.prepare_future_labels(data)
        loss, _, _ = model.shared_step(data, is_train=True)
        pass
        break
            ...
            # # for i in range(10):
            # #     depth_gt = F.max_pool2d(torch.from_numpy(depth_gt).unsqueeze(dim=0), kernel_size=3, stride=1, padding=1)
            # #     depth_gt = depth_gt.squeeze(0).numpy()

            # x, y = 0, 5
            # arr = np.zeros((15, 15, 5, 3))
            # for i in range(x - 7, x + 7):
            #     for j in range(y - 7, y + 7):
            #         for k in range(-2, 2):
            #             arr[i - x + 7, j - y + 7, k + 2] = np.array([i, j, k])

            # arr = arr.reshape(-1, 3)
            # arr = np.concatenate((arr, np.ones((arr.shape[0], 1))), axis=1)
            # arr = arr.T
            # pcarr = LidarPointCloud(arr)
            # pcarr.translate(-np.array(cs_record['translation']))
            # pcarr.rotate(Quaternion(cs_record['rotation']).rotation_matrix.T)
            # depths_imgarr = pcarr.points[2, :]
            # points = view_points(pcarr.points[:3, :], np.array(cs_record['camera_intrinsic']), normalize=True)
            # mask = np.ones(depths_imgarr.shape[0], dtype=bool)
            # mask = np.logical_and(mask, depths_imgarr > min_dist)
            ...
            # points = points[:, mask]
            # depths_imgarr = depths_imgarr[mask]

            # depth_gtarr = np.zeros((im.size[0], im.size[1]))
            # pts_int = np.array(points, dtype=int)
            # depth_gtarr[pts_int[0,:], pts_int[1,:]] = depths_imgarr
            # xs = [-1, 0, 1]
            # ys = [-1, 0, 1]
            # for x in xs:
            #     for y in ys:
            #         depth_gtarr[pts_int[0,:] + x, pts_int[1,:] + y] = depths_imgarr
