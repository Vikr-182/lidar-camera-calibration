...
class Planning(nn.Module):
    ...
    def loss(self, trajs, gt_trajs, cost_volume, semantic_pred, lane_divider, drivable_area, target_points):
        ...
        """
        https://arxiv.org/pdf/2101.06679.pdf
        Eq (8)
        max_{i \in N} [
            pos(
                {\sigma_t for t in T}(pos({\sigma_t for t \in T}(FO_it - GO_it)) )
                + FC_{i} - GC_{i} 
                + L2(s_{i} - s_gt) 
            )]
        (pos) means turned positive by ReLU
        """
        L, _ = F.relu(
            F.relu(gt_cost_fo - sm_cost_fo).sum(-1) + (gt_cost_fc - sm_cost_fc) + self.compute_L2(trajs, gt_trajs).mean(
                dim=-1)).max(dim=-1)

        return torch.mean(L), gt_cost_fc, gt_cost_fo
    ...
    def forward(self,cam_front, trajs, gt_trajs, cost_volume, semantic_pred, hd_map, commands, target_points):
        ...
        if self.training:
            loss, gt_cost_fc, gt_cost_fo = self.loss(cur_trajs, gt_trajs, cost_volume, semantic_pred, lane_divider, drivable_area, target_points)
        else:
            loss, gt_cost_fc, gt_cost_fo = self.loss(cur_trajs, gt_trajs, cost_volume, semantic_pred, lane_divider, drivable_area, target_points)
            loss = 0
        ...
        return loss, output_traj, CS, cur_trajs, gt_cost_fc, gt_cost_fo
