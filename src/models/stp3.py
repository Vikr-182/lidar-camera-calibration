...
from src.models.distributions import DistributionModule
...
class STP3(nn.Module):
    ...
    def forward(self, image, intrinsics, extrinsics, future_egomotion):
        ...
        F = 4/6 (future)
        N = 6 (num cameras)
        image: B, P, N, C, 224, 480
        intrinsics: B, P, N, 3, 3
        future_egomotion: B, P, 6
        x: B, P, 64, 200, 200
        depth: B, P, N, 48, 28, 60
        cam_front: B, P, 64, 28, 60
        """
        ...
        if self.cfg.MODEL.TEMPORAL_MODEL.INPUT_EGOPOSE:
            """
                future_egomotions_spatial: B, P, 6, 200, 200
                x:  B, P, 70, 200, 200 -> concatenated with future_egomotions_spatial
            """
        ...
        #  Temporal model
        """
        states: B, P, 64, 200, 200 -> encoding of past
        future_prediction_input: B, 1, 32, 200, 200
        """
        ...
        if self.n_future > 0:
            present_state = states[:, -1:].contiguous() # B, 1, 64, 200, 200
            if self.cfg.DEBUG: breakpoint()
            b, _, c, h, w = present_state.shape

            if self.cfg.PROBABILISTIC.ENABLED:
                if self.cfg.DEBUG: breakpoint()                
                future_prediction_input = self.distribution_forward(
                    present_state,
    ...
    def projection_to_birds_eye_view(self, x, geometry, future_egomotion):
        ...
        """
            x: (B, P, N, 48, 28, 60, 64)
            geometry: (B, P, N, 48, 28, 60, 64)
            future_egomotion: (B, P, 6) -> (6 dof movement for past)
        ...
        # Number of 3D points
        """
            N = 483840 (not to be confused with our N = num_cam)
            I will refer N=483840 as D for rest part.
        """
        
        N = n * d * h * w
        """
            !!!!
            why iterating over batch?
            use pack and unpack as used earlier
            !!!!
        """        
        for b in range(batch):
            """
                flow_b -> (P, N, 48, 28, 60, 64)
                flow_geo -> (P, N, 48, 28, 60, 64)
            """
            flow_b = x[b]
            flow_geo = geometry[b]
            ...
            for t in range(s):
                if t != s - 1:
                    """
                        flow_geo_tmp -> (t+1, N, 48, 28, 60, 64)
                        rotation_b -> (1, 1, 1, 1, 1, 3, 3)
                        translation_b -> (1, 1, 1, 1, 1, 3)
                    """
            ...
            for t in range(s):
                ...
                """
                    ...
                    geometry_b: (20496, 3)
                    x_b: (20496, 64)
                    tmp_bev_feature: (1, 200, 200, 64)
                    bev_feature: (1, 200, 200, 64)
                """
    ...
    def calculate_birds_eye_view_features(self, x, intrinsics, extrinsics, future_egomotion):
        ...
        """
            x: (B, P, N, C, H, W)
            x (packed): (B * P, N, C, H, W)

            intrinsics: (B, P, N, 3, 3)
            extrinsics: (B, P, N, 4, 4) -> R, T combined (4x4) matrix for each
        """
        # Reshape
