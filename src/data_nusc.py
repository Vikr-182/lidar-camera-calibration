...
from src.utils.tools import ( gen_dx_bx, get_nusc_maps)
...
import src.utils.sampler as trajectory_sampler
...
class FuturePredictionDataset(torch.utils.data.Dataset):
    ...
    def prepro(self):
        ...
        for cam in self.cfg.IMAGE.NAMES: samples = [rec for rec in samples if os.path.exists(os.path.join(self.nusc.dataroot, self.nusc.get('sample_data', rec['data'][cam])['filename']))];print(len(samples), "COI:A")
    ...
    def get_centerline(self, rec, index_t):
        ...
        closest_lane = self.nusc_maps[map_name].get_closest_lane(center[0], center[1], radius=2)
        lane_neighbours = self.nusc_maps[map_name].get_outgoing_lane_ids(closest_lane)
        lidar_sample_data = self.nusc.get('sample_data', rec['data']['LIDAR_TOP'])
        sd_ep = self.nusc.get("ego_pose", lidar_sample_data["ego_pose_token"])
        sd_cs = self.nusc.get("calibrated_sensor", lidar_sample_data["calibrated_sensor_token"])

        lane_record = self.nusc_maps[map_name].get_arcline_path(closest_lane)
        closest_pose_on_lane, distance_along_lane = arcline_path_utils.project_pose_to_lane((center[0], center[1], yaw), lane_record)
        poses = arcline_path_utils.discretize_lane(lane_record, resolution_meters=0.5)
        whe = np.where((poses - np.array(closest_pose_on_lane)) == [0.0, 0.0, 0.0])
        ind = whe[0][0]
        # print(ind, "ind", closest_pose_on_lane)
        lane_poses = poses[ind:]
        n_output = self.cfg.N_FUTURE_FRAMES
        gt_lane_poses = np.zeros_like(lane_poses)

        # from sensor to global
        ...
        init_pose = sensor_from_ego.dot(ego_from_global)
        centerlines = []

        for ind, pose in enumerate(lane_poses):
            lanepose = transform_matrix(pose, Quaternion([0,0,0,0]), inverse=False)
            lanepose = init_pose.dot(lanepose)
            origin = np.array(lanepose[:3, 3])
            gt_lane_poses[ind, :] = [origin[0], origin[1], 0.0]

        import copy
        for lane in lane_neighbours:
            lane_record = self.nusc_maps[map_name].get_arcline_path(lane)
            closest_pose_on_lane, distance_along_lane = arcline_path_utils.project_pose_to_lane((center[0], center[1], yaw), lane_record)
            poses = arcline_path_utils.discretize_lane(lane_record, resolution_meters=0.5)
            whe = np.where((poses - np.array(closest_pose_on_lane)) == [0.0, 0.0, 0.0])
            ind = whe[0][0]
            ...
            centerline = copy.deepcopy(gt_lane_poses)
            for ind, pose in enumerate(lane_poses):
                lanepose = transform_matrix(pose, Quaternion([0,0,0,0]), inverse=False)
                lanepose = init_pose.dot(lanepose)
                origin = np.array(lanepose[:3, 3])
                centerline = np.vstack((centerline,[origin[0], origin[1], 0.0]))
    ...
    def voxelize_hd_map(self, rec):
        ...
        box_coords = ( center[0], center[1], stretch[0]*2, stretch[1]*2 ) # (x_center, y_center, width, height)
        canvas_size = (int(self.cfg.LIFT.X_BOUND[1] * 2 / self.cfg.LIFT.X_BOUND[2]), int(self.cfg.LIFT.Y_BOUND[1] * 2 / self.cfg.LIFT.Y_BOUND[2]))
    ...
    def __getitem__(self, index):
        ...
        keys = ['image', 'intrinsics', 'extrinsics', 'depths',
                ...
                'future_egomotion', 'hdmap', 'gt_trajectory', 'indices', 'closest_lane_poses'
                ]
        for key in keys:
            data[key] = []
        ...
        for i, index_t in enumerate(self.indices[index]):
            ...
            plt.imshow(combined);plt.savefig(f"output_vis/{index_t}.png");plt.clf()
            """
            ...
            if i == self.cfg.TIME_RECEPTIVE_FIELD-1:
                ...
                data['closest_lane_poses'] = self.get_centerline( rec, index_t)
                print(np.array(self.get_centerline( rec, index_t)).shape)
