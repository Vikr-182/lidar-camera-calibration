...
class SegmentationLoss(nn.Module):
    ...
    def forward(self, prediction, target, n_present=3):
        """
            prediction: (B, 9, 2, 200, 200)
            target: (B, 9, 1, 200, 200)
        """
        if target.shape[-3] != 1:
        ...
        """
            cross_entropy: on dim 3
            loss: (B, 9, 200, 200)
        """
        loss = F.cross_entropy(prediction,target,ignore_index=self.ignore_index,reduction='none',weight=self.class_weights.to(target.device),)
        ...
        """
            discount the future loss
            penalize the present and past loss
            discounts: (B, 9, 1, 1)
            loss: (B, 9, 200, 200)
        """
        future_discounts = self.future_discount ** torch.arange(1, future_len+1, device=loss.device, dtype=loss.dtype)
        ...
        if self.use_top_k:
            """
                penalize top-k (0.25 here) cases
                loss: (B, 9, 10000)
            """
...
class HDmapLoss(nn.Module):
    ...
    def forward(self, prediction, target):
        """
            prediction: (B, 4, 200, 200)
            target: (B, 2, 200, 200)
        """
        loss = 0
        for i in range(target.shape[-3]): # do road and lane dividers
            cur_target = target[:, i]
            b, h, w = cur_target.shape
            cur_prediction = prediction[:, 2*i:2*(i+1)] # (B, 2, 200, 200)
